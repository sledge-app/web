...
window.$sledge.equalizerRefresh = function (viewRef) {
  ...
  if (viewRef) {
    ...
    var views = document.body.querySelectorAll('.sg_view');[].forEach.call(views, function (view) {
      equalizer(view);
    });
  }
...
function equalizer(view) {
  var rows = view.querySelectorAll('.sg_row');[].forEach.call(rows, function (row) {
    equalizerRow(row);
  });
}
...
// global
window.$sledge.equalizerSingleRow = function (row) {
  var reset = true;
  ...
  if (reset) {
    window.requestAnimationFrame(function (a) {
      return window.Foundation.reInit('equalizer');
    });
  } else {
    $(row).foundation();
  }
...
function equalizerRow(row) {
  ...
  var fullSize = 0;[].forEach.call(cols, function (col) {
    var size = (0, _utilities.getColumnSizeFromString)(col.className);
    var offsetSize = (0, _utilities.getColumnOffsetFromString)(col.className);
    var full = size + offsetSize;
    fullSize += full;
...
window.addEventListener('resize', function () {
  ...
  timeout = setTimeout(function () {
    return window.$sledge.equalizerRefresh();
  }, delay);
});
...
exports.getColumnOffsetFromString = getColumnOffsetFromString;
exports.getColumnSizeFromString = getColumnSizeFromString;
exports.addMediaForExport = addMediaForExport;
...
window.addEventListener('resize', function () {
  clearTimeout(timeout);
  timeout = setTimeout(function () {
    return window.$sledge.refreshOrbit();
  }, delay);
});
...
/***/ })
/******/ ]);
//# sourceMappingURL=bundle_runtime.js.map